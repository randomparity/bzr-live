...
`bug.worktime` share `append` while carrying different `reconcile` bodies and matching on
different reply fields (`text` versus `summary`,
`src/bzr_live/replay/actions.py:459-465`, `:504-514`, `:535-542`).
...
an assertion that can fail. It models only the commands the five actions' handlers issue,
answers with the reply shapes those handlers read, and refuses any other command with an
explicit failure rather than a default.

**All five actions are covered, not the three classes they collapse into**, and
`bug.update` is covered twice: once where the declared fields read back (reconciliation
answers `advance`, nothing is re-sent) and once through the fixture's own `update-triage`,
whose `remaining_hours` `bzr bug view` never serializes (reconciliation answers `retry`
and `resume` re-applies the idempotent set at attempt 2).

## Consequences

- The double is a second model of the boundary and can drift from the real one. It is
  bounded on purpose: it proves *engine recovery*, never bzr's or Bugzilla's behaviour.
  The live proof of boundary fidelity stays `tests/replay_smoke.sh` and the observed run
  ADR 0006 records.
...
- **Cover the three recovery classes instead of the five actions.** verified:
  ...
  (`src/bzr_live/replay/actions.py:459-465`, `:504-514`, `:535-542`) matching a different
  reply field, so one append case leaves two reconcilers unexercised.
