...
3. In the Dockerfile, use the pinned Ubuntu index, install upstream-required
   Apache/Perl/MariaDB-client packages from Ubuntu, fetch the exact DBD MariaDB,
   Template Toolkit, and PatchReader archives and verify their SHA-256 values
   before installation, download the canonical Bugzilla commit archive and
   verify its exact SHA-256 before extraction, configure Apache modules, and
   copy only the owned setup files.
